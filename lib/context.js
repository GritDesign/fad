...
Context.prototype._eachContext = function (cb) {
	var index = 0;
	...
	for (var i = 0; cont && i < this._parents.length; i++) {
		this._parents[i]._eachContext(eachCallback);
	}
};

Context.prototype._getContext = function (index) {
	if (index === 0) {
		return this;
	}
	...
	for (var i = 0; i < this._parents.length; i++) {
		if (index < this._parents[i]._contextCount) {
			return this._parents[i]._getContext(index);
		}
...
Context.prototype._smallestEnclosingContext = function (minCtx, maxCtx) {
	if (minCtx === 0) {
		return this;
	}
	...
	for (var i = 0; i < this._parents.length; i++) {
		...
		if (minCtx >= min && maxCtx < max) {
			return parent._smallestEnclosingContext(min - ctxCount,
				max - ctxCount);
		}
...
Context.prototype.get = function (key, skipRules, cb) {
	...
	params.forEach(function (param, index) {
		...
		var deps = self._getDeps(param, [], skipRules);
		...
		if (deps.type === "value") {
			count--;
			results[index] = self._getContext(deps.ctxIndex)._values[param];
			checkDone();
			return;
		}
		...
		if (deps.type === "rule") {
			...
			var ctx = self._smallestEnclosingContext(deps.minCtx,
				deps.maxCtx);
			...
			if (HOP(ctx._resolving, param) && noSkip) {
				...
				var rule = self._getContext(deps.ctxIndex)
...
Context.prototype._getRdeps = function (name, collect) {
	var self = this;
...
Context.prototype._getDeps = function (name, seen, skipRules) {
	var self = this;
	...
	self._eachContext(function (ctx, ctxIndex) {
		...
		if (HOP(ctx._rules, name)) {
			for (var ri = 0; ri < ctx._rules[name].length; ri++) {
				...
				for (var di = 0; di < rule.deps.length; di++) {
					...
					var dep = self._getDeps(param, newSeen, skipClone);
					if (!dep.hasDeps) {
						hasDeps = false;
					}
...
Context.prototype._setDirty = function (name) {
	...
	if (!self._willEmit) {
		...
		immediate(function () {
			var allDirtyValues = [];
			for (var key in self._dirty) {
				if (HOP(self._dirty, key)) {
					allDirtyValues.push(key);
				}
			}

			var handlers = self._handlers.dirty;
			if (handlers) {
				handlers.forEach(function (handler) {
					if (handler.selector === "*") {
						handler.handler(allDirtyValues);
					} else {
						var dirtyValues = [];
						var selected = handler.selector.split(" ");
						for (var i = 0; i < selected.length; i++) {
							if (HOP(self._dirty, selected[i])) {
								dirtyValues.push(selected[i]);
							}
						}
						if (dirtyValues.length) {
							handler.handler(dirtyValues);
						}
					}
				});
			}

			self._willEmit = false;
		});
	}
};
...
Context.prototype.keys = function () {
	...
	this._eachContext(function (ctx) {
...
Context.prototype.on = function (type, selector, handler) {
	if (type !== "dirty") {
...
Context.prototype.off = function (type, selector, handler) {
	if (type !== "dirty") {
		throw new Error("Invalid event type");
	}
	...
	for (var i = 0; i < handlers.length; i++) {
		if (handlers[i].selector !== selector ||
			handlers[i].handler !== handler) {
			newHandlers.push(handlers[i]);
		}
	}

	this._handlers[type] = newHandlers;
};
