...
Context.prototype.get = function (key, skipRules, cb) {
	...
	params.forEach(function (param, index) {
		...
		if (deps.type === "rule") {
			var resolvingKey;
			if (skipRules[param]) {
				resolvingKey = param + ":0";
			} else {
				resolvingKey = param + ":" + skipRules[param];
			}
			...
			if (HOP(ctx._resolved, resolvingKey)) {
				count--;
				results[index] = ctx._resolved[resolvingKey];
				checkDone();
				return;
			}

			if (HOP(ctx._resolving, resolvingKey)) {
				ctx._resolving[resolvingKey].push(resolved);
				return;
			} else {
				ctx._resolving[resolvingKey] = [];
				...
				ctx.get(rule.deps, skipClone, function (err, results) {
					if (err) {
						ctx._resolving[resolvingKey].forEach(function (fn) {
							fn(err);
						});
						return;
					}
					...
					results.push(function (err, value) {
						if (!err) {
							ctx._resolved[resolvingKey] = value;
						}
						ctx._resolving[resolvingKey].forEach(function (fn) {
							fn(err, value);
						});
						delete ctx._resolving[resolvingKey];
					});
